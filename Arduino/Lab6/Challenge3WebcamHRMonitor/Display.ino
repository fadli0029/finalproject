...
*/
void writeDisplay(const char * message, int row, bool erase, bool sec5) {
  unsigned long now = millis();
  //if erase is true and it's been longer than MAX_REFRESH (1s)
  if (erase && (sec5 == false) && (millis() - lastClear >= MAX_REFRESH)) {
    oled.clearDisplay();
    lastClear = now;
  }
  else if ((erase == false) && (sec5 == true) && (millis() - lastClear >= (MAX_REFRESH)*5)) {
    oled.clearDisplay();
    lastClear = now;
  }
...
void writeDisplayCSV(String message, int commaCount) {
  ...
  for (int i=0; i<=commaCount; i++) {
    ...
    writeDisplay(subMessage.c_str(), i, false, false);
  }
}
