...
if __name__ == "__main__":
  ...
  try:
    previous_time = time()
    sleep(2) # so i can skip first peak due to pickup up the watch
    # or you can just remove the first few seconds of data?
    while(True):
      ...
      if(message != None):
        ...
        if (current_time - previous_time > process_time):
          ...
          plt.cla()
          plt.plot(filtered)
          plt.title("Muhammad Arsani | Step Count: %d" % steps)
